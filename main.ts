...
export default class FullwidthTablePlugin extends Plugin {
	...
	constructor(app: App, manifest: PluginManifest) {
		...
		this.tableMap.addChangeEventListener(
			"tableWrapperWidth",
			(tableId, width) => {
				console.log(
					`tableWrapperWidth of ${tableId} updated: ${width}`
				);
			}
		);
		...
		this.tableMap.addChangeEventListener("leftGap", (tableId, left) => {
			...
			if (!(tableWidgetElement instanceof HTMLElement)) {
				throw new Error(
					`tableWidgetElement is not an HTMLElement: ${tableId}`
				);
			}

			tableWidgetElement.style.setProperty(
				"--plugin-fullwidth-table-left-gap",
				`${left}px`
			);
		});
		...
		this.tableMap.addChangeEventListener("viewWidth", (width) => {
			document.body.style.setProperty(
				"--plugin-fullwidth-table-view-width",
				`${width}px`
			);
		});
	}
	...
	override async onload() {
		this.app.workspace.onLayoutReady(() => {
			...
			if (
				!viewContentElement ||
				!(viewContentElement instanceof HTMLElement)
			) {
			...
			const tableInsertionObserver = new MutationObserver((mutations) => {
				...
				const addedTableWidgets = new Set(
					mutations
						...
						.filter((node) =>
							node.classList.contains("cm-table-widget")
						)
				);

				console.log(addedTableWidgets);

				addedTableWidgets.forEach((tableWidgetElement) =>
					this.initTable(tableWidgetElement as HTMLElement)
				);
			});
	...
	private init() {
		const tableWidgetElements = Array.from(
			document.querySelectorAll("div.cm-table-widget")
		);
		console.log(
			`Found ${tableWidgetElements.length} div.cm-table-widget elements`
		);
		tableWidgetElements.forEach((tableWidgetElement) =>
			this.initTable(tableWidgetElement as HTMLElement)
		);
	}
	...
	private setTableWrapperWidth(tableWidgetElement: HTMLElement) {
		...
		const tableWrapperElement =
			tableWidgetElement.querySelector("div.table-wrapper");
		if (
			!tableWrapperElement ||
			!(tableWrapperElement instanceof HTMLElement)
		) {
...
class TableMap {
	...
	private calcLeftGap(tableId: TableId) {
		...
		if (tableWrapperWidth < lineWidth) {
			...
			lineWidth < tableWrapperWidth &&
			tableWrapperWidth < this.viewWidth
		) {
	...
	addChangeEventListener<T extends keyof ChangeEventListeners>(
		event: T,
		callback: ChangeEventListeners[T][number]
	) {
